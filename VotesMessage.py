    ...
    def get_message(self, selected_items, votes):
        ...
        for i, selected_value in enumerate(selected_items):
            ...
            lunchVoteResult = [
                ...
                {
                    "type": "mrkdwn",
                    "text": f"*<{lunchRow[2]}|{lunchRow[1]}>*\n{lunchRow[4] if len(lunchRow[4])<20 else f'{lunchRow[4].split()[0]} {lunchRow[4].split()[-1:][0]}'} *{lunchRow[5]}*"
                    if lunchRow[2] else f"*{lunchRow[1]}*\n{lunchRow[4]} *{lunchRow[5]}*"
                },
                {
                    "type": "mrkdwn",
                    # e.g. votes[i]: [up,dn]
                    "text": f"*Votes*\n:thumbsup: {votes[i][0]}\t\t:thumbsdown: {votes[i][1]}"
                }
            ]
            # appends to fields by adding 4 dicts to list everytime
